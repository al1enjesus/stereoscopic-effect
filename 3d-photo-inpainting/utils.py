...
def get_MiDaS_samples(image_folder, depth_folder, config, specific=None, aft_certain=None):
    ...
    assert config['num_frames'] == len(config['x_shift_range']) and \
           (len(config['y_shift_range']) == len(config['z_shift_range']) == len(config['x_shift_range'])), \
           "The number of elements in 'x_shift_range', 'y_shift_range', 'z_shift_range' and \
               'num_frames' should be equal."
    tgt_pose = [[generic_pose * 1]]
    tgts_poses = []
    for traj_idx in range(len(config['traj_types'])):
        tgt_poses = []
        sx = config['x_shift_range']
        sy = config['y_shift_range']
        sz = config['z_shift_range']
